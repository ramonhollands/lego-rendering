...
class Renderer:
    ...
    def create_background_plane(self):

        # TODO download more : https://www.pexels.com/search/texture/

        # 1. Create a Plane
        bpy.ops.mesh.primitive_plane_add(size=0.6, enter_editmode=False, align='WORLD', location=(0, 0, 0))
        ...
        images_in_dir = os.listdir("/home/ramon/blender/backgrounds")
        random_image = random.choice(images_in_dir)
        
         # Replace with the path to your image file
        image = bpy.data.images.load('/home/ramon/blender/backgrounds/' + random_image)
        ...
        # 5. Assign the Material to the Plane
    ...
    def render_parts(self, ldraw_part_ids, multi_options):

        options = multi_options[0]
        ...
        for i in range(len(ldraw_part_ids)):
            clear_scene = i == 0    
            self.import_part(ldraw_part_ids[i], multi_options[i], clear_scene=clear_scene, color_index=i)
            succesfull_ids.append(ldraw_part_ids[i])
            

        print('Importing done')
                
        ldraw_part_ids = succesfull_ids

        parts = []
        for i in range(len(ldraw_part_ids)):
            parts.append(bpy.data.objects[i].children[0])
            color_names.append(multi_options[i].color_name)
        ...
        for i in range(len(parts)):
            rotation = multi_options[i].part_rotation_radian
            ...
            random_move_x = random.uniform(-0.1, 0.2)
            random_move_y = random.uniform(-0.1, 0.2)
            random_move_z = None #random.uniform(0, 0.1)
            random_moves.append((random_move_x, random_move_y))
            place_object_on_ground(part, move_x=random_move_x, move_y=random_move_y, move_z=random_move_z)
            ...
            with open(options.bounding_box_filename, 'w') as f:
                for i in range(len(parts)):
                    try:
                        ...
                        bounding_box = get_2d_bounding_box(part, camera).to_box21(options.width, options.height)
                        f.write(f"{color_name} {bounding_box[0]:.3f} {bounding_box[1]:.3f} {bounding_box[2]:.3f} {bounding_box[3]:.3f}\n")
                    except Exception as e:
                        print(e)

    def render_part(self, ldraw_part_id, options):

        print("Rendering part", ldraw_part_id)
        ...
        if dimensions.x > 0.1 or dimensions.y > 0.1:
            print(f"Skipping part {part.name} due to size constraints.")
        else:
    ...
    def import_part(self, ldraw_part_id, options, clear_scene=True, color_index=1):
        if clear_scene:
            self.clear_scene()
        ...
        if options.material == Material.TRANSPARENT:
            ldraw_color = 36
        elif options.material == Material.RUBBER:
            ldraw_color = 256
        else:
            ldraw_color = color_index
